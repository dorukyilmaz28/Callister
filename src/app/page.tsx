...
import { Award, Users, Lightbulb, Heart, Brain, Zap, Target, Code, GraduationCap, Layout, Cpu } from 'lucide-react'
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* FRC Academy Section - Callister AI ile aynı yapı, daha dikkat çekici */}
      <section className="relative py-12 sm:py-16 md:py-20 bg-gradient-to-br from-primary via-dark to-accent overflow-hidden">
        {/* Arka plan vurgusu */}
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[400px] bg-purple-500/10 rounded-full blur-[100px]" />
        </div>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          <div className="text-center mb-10 sm:mb-12">
            <span className="frc-academy-badge">{t('frcAcademySection.badge') as string}</span>
            <h2 className="section-title frc-academy-title-gradient drop-shadow-lg">{t('frcAcademySection.title') as string}</h2>
            <p className="section-subtitle text-[#F5F5F5]/90 font-medium">
              {t('frcAcademySection.subtitle') as string}
            </p>
          </div>

          <div className="text-center mb-12 sm:mb-16">
            <h3 className="text-2xl sm:text-3xl font-bold text-[#F5F5F5] mb-4 sm:mb-6 drop-shadow-md">
              {t('frcAcademySection.heading') as string}
            </h3>
            <p className="text-lg sm:text-xl text-[#F5F5F5]/90 mb-6 sm:mb-8 font-medium">
              {t('frcAcademySection.subheading') as string}
            </p>
            ...
            <div className="max-w-4xl mx-auto space-y-6 text-sm sm:text-base text-[#F5F5F5]/95 font-medium leading-relaxed font-['Poppins']">
              <p className="text-base sm:text-lg">
                {t('frcAcademySection.paragraph1') as string}
              </p>

              <div className="space-y-3 sm:space-y-4">
                <div className="flex items-center justify-center space-x-3">
                  <span className="text-2xl">🔹</span>
                  <p className="text-sm sm:text-base">{t('frcAcademySection.feature1') as string}</p>
                </div>
                <div className="flex items-center justify-center space-x-3">
                  <span className="text-2xl">🔹</span>
                  <p className="text-sm sm:text-base">{t('frcAcademySection.feature2') as string}</p>
                </div>
                <div className="flex items-center justify-center space-x-3">
                  <span className="text-2xl">🔹</span>
                  <p className="text-sm sm:text-base">{t('frcAcademySection.feature3') as string}</p>
                </div>
              </div>

              <p className="text-base sm:text-lg font-semibold mt-6">
                {t('frcAcademySection.conclusion') as string}
              </p>
            </div>
            ...
            <div className="mt-8 sm:mt-10 flex justify-center">
              ...
              >
                <button className="btn-primary btn-frc-academy flex items-center justify-center space-x-2 px-8 py-4 text-base">
                  <GraduationCap size={22} />
                  <span>{t('frcAcademySection.button') as string}</span>
                </button>
              </a>
            </div>
          </div>

          {/* FRC Academy Features Grid - 4 kart */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
            {[
              { icon: Code, titleKey: 'frcAcademySection.features.software.title', descriptionKey: 'frcAcademySection.features.software.description' },
              { icon: Layout, titleKey: 'frcAcademySection.features.design.title', descriptionKey: 'frcAcademySection.features.design.description' },
              { icon: Zap, titleKey: 'frcAcademySection.features.mechanical.title', descriptionKey: 'frcAcademySection.features.mechanical.description' },
              { icon: Cpu, titleKey: 'frcAcademySection.features.electronics.title', descriptionKey: 'frcAcademySection.features.electronics.description' },
            ].map((feature, index) => (
              <div key={index} className="card card-frc-academy text-center">
