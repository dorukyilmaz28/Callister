...
export default function ProjectsPage() {
  ...
  const getProjectTranslation = (projectTitle: string) => {
    const translations: { [key: string]: any } = {
      ...
      'Callister AI': {
        title: t('projects.projectDetails.callisterAi.title') as string,
        description: t('projects.projectDetails.callisterAi.description') as string,
        details: t('projects.projectDetails.callisterAi.details') as string
      },
      'YAGSL': {
      ...
      <div className="relative z-10 min-h-screen flex flex-col">
        <div className="flex-1 py-20">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {/* Yaptığımız Bağışlar - sayfanın en altında */}
            <section className="pt-8 border-t border-white/20">
              <h2 className="text-2xl sm:text-3xl font-bold text-white mb-6 text-center">
                {t('projects.donations.title') as string}
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                {[
                  { src: '/images/projects/bagis1.png', fallback: 'Fotoğraf 1' },
                  { src: '/images/projects/bagis2.png', fallback: 'Fotoğraf 2' },
                  { src: '/images/projects/bagis3.png', fallback: 'Fotoğraf 3' }
                ].map((item, num) => (
                  <div
                    key={num}
              ...
              <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 sm:p-8 border border-white/20 space-y-4">
                <p className="text-gray-200 leading-relaxed">
                  {t('projects.donations.paragraph1') as string}
                </p>
                <p className="text-gray-200 leading-relaxed">
                  {t('projects.donations.paragraph2') as string}
                </p>
                <p className="text-gray-200 leading-relaxed">
                  {t('projects.donations.paragraph3') as string}
                </p>
                <p className="text-gray-200 leading-relaxed">
            ...
            {/* Düzenlediğimiz Kermesler - Bağışların altında, 2 görsel */}
            <section className="pt-10 border-t border-white/20 mt-10">
              <h2 className="text-2xl sm:text-3xl font-bold text-white mb-6 text-center">
                {t('projects.kermes.title') as string}
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                {[
                  { src: '/images/projects/duzenledigimiz-kermesler1.png', fallback: 'Kermes 1' },
                  { src: '/images/projects/duzenledigimiz-kermesler2.png', fallback: 'Kermes 2' }
                ].map((item, num) => (
                  <div
                    key={num}
                    className="aspect-[4/3] bg-white/10 backdrop-blur-md rounded-xl border border-white/20 overflow-hidden relative"
              ...
              <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 sm:p-8 border border-white/20 space-y-4">
                <p className="text-gray-200 leading-relaxed">
                  {t('projects.kermes.paragraph1') as string}
                </p>
                <p className="text-gray-200 leading-relaxed">
                  {t('projects.kermes.paragraph2') as string}
                </p>
                <p className="text-gray-200 leading-relaxed">
                  {t('projects.kermes.paragraph3') as string}
                </p>
              </div>
            </section>
          </div>
            ...
            <div className="overflow-y-auto max-h-[calc(100vh-200px)]">
              ...
              {(modalProject.title === 'FRC Academy' || modalProject.title === 'Callister AI' || modalProject.title === 'YAGSL') && (
                <p className="mb-4">
                  <a
                    href={
                      modalProject.title === 'FRC Academy'
                    ...
                    className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-white/20 hover:bg-white/30 text-white font-medium border border-white/30 transition-colors"
                  >
                    <ArrowRight className="w-4 h-4" />
                    {t('projects.siteButton') as string}
                  </a>
                </p>
              )}
              {/* Detay görselleri - her biri kendi oranında, boşluk kalmaz */}
